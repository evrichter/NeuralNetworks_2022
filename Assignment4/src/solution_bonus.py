...
x_train, x_dev, y_train, y_dev = train_test_split(data, target, train_size=1000, test_size=100, random_state=25)
...
def start(l):
    ...
    for t in range(epochs):
        #print(f"Epoch {t+1}\n-------------------------------")
        liste.append((train_loop(train_loader, net, loss_fn, optimizer),test_loop(dev_loader, net, loss_fn)))
    print("Done!",l)
    plt.plot(range(epochs),liste)
    plt.show()
